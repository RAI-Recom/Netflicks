...
# from db.db_manager import DBManager
# from sklearn.metrics import mean_squared_error
# from pipeline.hybrid_recommender import hybrid_recommend  # ✅ direct import
...
# #print("Starting direct model evaluation (no API)...")
# for user_id in tqdm(user_ids, desc="Querying Recommender"):
#     start_time = time.time()
#     recommendations = hybrid_recommend(user_id, top_n=K)
#     latency = time.time() - start_time
...
# # Throughput
# total_time_sec = sum(latencies)
# metrics["throughput_rps"] = round(n_users / total_time_sec, 2)

# # SLA Violation (optional)
# sla_threshold_ms = 300
# sla_violations = [1 for l in latencies_ms if l > sla_threshold_ms]
# metrics["sla_violation_rate"] = round(len(sla_violations) / n_users, 4)

# # Recommendation counts
# metrics["mean_recommendations_per_user"] = round(np.mean(recommendation_counts), 2)

# # --- SAVE RESULTS ---
# print("\n=== Final Direct Evaluation Metrics ===")
# for k, v in metrics.items():
#     print(f"{k}: {v}")
